...
UNITS = [
    ...
    ("chars",
     ["-chars=abcde length=5 size=1", "-chars=ab̤de length=5 size=2",
...
RE_BLO=r'BYTEA|BLOB'
RE_INT=r'{0}|(TINY|SMALL|MEDIUM)INT|INT[248]|INTEGER|MONEY|INT\b'.format(RE_SER)
RE_FLT=r'REAL|FLOAT|DOUBLE\s+PRECISION|NUMERIC|DECIMAL|MONEY'
RE_TXT=r'TEXT|CHAR\(\d+\)|VARCHAR\(\d+\)'
RE_BIT=r'BIT\(\d+\)|VARBIT\(\d+\)'
RE_TSTZ=r'TIghgffghMESTAMP(\s+WITH\s+TIME\s+ZONE)?' # lol we dont want this to generate cos we want default values k thx
RE_INTV=r'INTERVAL'
